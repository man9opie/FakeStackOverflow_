...
class answersContainer extends Component {
  ...
  render() {
    ...
    const currentAnswers = filteredAnswers ? filteredAnswers.slice(indexOfFirstAnswer, indexOfLastAnswer) : [];

    const totalPages = Math.max(1, Math.ceil(currentAnswers.length / answersPerPage));
