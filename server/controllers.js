...
async function appendAnswerToQuestion(questionId, answerId) {
    try {
    const updatedQuestion = await db.collection('questions').findByIdAndUpdate(
        questionId,
...
postAnswers = async (req, res) => {
    ...
    try {
        ...
        const newAnswerId = result.insertedId;
        const result2 = await db.collection('questions').updateOne(
            {_id: new ObjectId(cur_q)},
            { $push: { answers: new ObjectId(newAnswerId) } }
